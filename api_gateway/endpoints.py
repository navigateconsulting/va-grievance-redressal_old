...
from models import ProjectsModel, DomainsModel, IntentsModel, ResponseModel, StoryModel, EntityModel, RefreshDb, RasaConversations, CustomActionsModel, GrievanceModel
...
@sio.on('getGrievance', namespace='/grievance')
async def get_grievance(sid, room_name):

    print("---------- Request from Session {} -------------- ".format(sid))

    result = await GrievanceModel.get_grievance()
    await sio.emit('allGrievance', result, namespace='/grievance', room=room_name)

@sio.on('getAGrievance', namespace='/grievance')
async def get_grievance(sid, grievance_id, room_name):

    print("---------- Request from Session {} -------------- ".format(sid))

    result = await RasaConversations.get_conversations(grievance_id)
    await sio.emit('aGrievance', result, namespace='/grievance', room=room_name)
